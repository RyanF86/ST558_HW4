...
head(parsed$articles) # this is the data frame with information for the articles
str(parsed$articles) # here is the structure of the articles data frame
parsed$articles$content[1] # this is the actual text from the first article
```
...
```{r}
head(NewsAPIQuery("Disneyland", "2026-06-08", RFKey))
```

Testing function with search of "Tokyo" from "2026-06-04".

```{r}
head(NewsAPIQuery("Tokyo", "2026-06-04", RFKey))
```
...
CoasterDataRaw 
describe(CoasterDataRaw) # shows summary statistics on all variables in order to look for anything awry
```

- Nothing unusual identified in looking at mean, min, and max with describe()
